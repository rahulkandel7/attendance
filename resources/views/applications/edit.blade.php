...
@section('content')
    ...
    <div class="w-full h-screen bg-opacity-60 ">
        <h1 class="text-2xl pt-8 pb-2 text-indigo-500 font-bold">
            ...
            <form action="{{route('applications.update',$leave->id)}}" method="post" class="w-[90%]" enctype="multipart/form-data">
                @method('put')
                @csrf
				...
				<div class="my-2 py-2 mx-6">
                    ...
                    <input type="text" class="rounded-lg  shadow-lg border border-transparent focus:ring-gray-200 w-full  focus:border-gray-600 " required name="subject" placeholder="Subject" value="{{$leave->subject}}">
                </div>

                <div class="my-2 py-2 mx-6 ">
                    <label class="py-2 text-gray-500 font-bold my-2">Application Letter</label>

                    <textarea class="rounded-lg  shadow-lg border border-transparent focus:ring-gray-200 w-full  focus:border-gray-600 editor" name="description"  >{{$leave->description}}</textarea>
                </div>
                ...
                @if (Auth::user()->role == "admin")
                    <div class="my-2 py-2 mx-6">
                        <label class="py-2 text-gray-500 font-bold my-2">Status</label>

                        <select class="rounded-lg  shadow-lg border border-transparent focus:ring-gray-200 w-full  focus:border-gray-600 file:rounded-md file:bg-indigo-500 file:border-none file:text-white" name="status" >
